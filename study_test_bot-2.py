...
try:
    ...
    # ── الست صفحات الجديدة لهيكلة البيانات ──────────────
...
# ─── Cache للبيانات الرئيسية (sheet data القديم) — تلقائي كل 60 ثانية ───
# ملاحظة: هذا الكاش خاص بالشيت القديم (sheet1) فقط، يبقى مؤقتاً لحين قرار الهجرة
_sheet_data_cache = {"data": None, "ts": 0}

# ─── Cache موحد للست صفحات الجديدة — يدوي/عند الكتابة فقط، بدون TTL ───
# يتحدّث فقط بحالتين: أمر /refresh_data من بوت اللوج، أو مباشرة بعد أي عملية
# إضافة/تعديل/حذف بيانات من البوت نفسه (استدعاء invalidate_data_cache)
_DATA_TABS = {
...
_data_cache = {key: {"data": None, "ts": 0} for key in _DATA_TABS}
...
def invalidate_sheet_cache():
    """إبطال cache الشيت القديم (sheet1) فوراً — عند الكتابة أو طلب تحديث يدوي"""
    _sheet_data_cache["data"] = None
    _sheet_data_cache["ts"]   = 0

def get_tab_data(tab_key: str) -> list:
    """يقرأ صفوف صفحة بيانات (محاضرات/ملازم/ملخصات/تكاليف/نماذج/targets) من الكاش
    الموحد، ويجلبها من الشيت فقط لو الكاش فاضي (بدون TTL — يدوي/عند الكتابة فقط)."""
    cache = _data_cache.get(tab_key)
    ws    = _DATA_TABS.get(tab_key)
    if cache is None or ws is None:
        return []
    if cache["data"] is not None:
        return cache["data"]
    try:
        rows = ws.get_all_values()[1:]
        useful = [r for r in rows if any(c.strip() for c in r if c)]
        cache["data"] = useful
        cache["ts"]   = time.time()
        return useful
    except Exception as e:
        log_error(f"get_tab_data({tab_key}): {e}")
        return cache["data"] or []

def invalidate_data_cache(tab_key: str = None):
    """إبطال كاش صفحة بيانات واحدة، أو كل الصفحات الست لو ما تحدد tab_key.
    يُستدعى مباشرة بعد أي عملية إضافة/تعديل/حذف، أو من أمر /refresh_data."""
    keys = [tab_key] if tab_key else list(_data_cache.keys())
    for k in keys:
        if k in _data_cache:
            _data_cache[k]["data"] = None
            _data_cache[k]["ts"]   = 0
...
def get_subjects_with_tasks():
    """يرجع أسماء المواد التي لديها تكليف واحد على الأقل، بترتيب أول ظهور بالشيت."""
    try:
        seen = []
...
def get_tasks_for_subject(subject):
    """يرجع تكاليف مادة معينة بترتيب الإدخال (ترتيب الصفوف بالشيت)."""
    try:
        tasks = []
        for r in get_tab_data("assignments"):
...
def save_task(subject, name, text_val="", file_ids=None, alert=""):
    """يضيف تكليف جديد، أو يدمج الملفات الجديدة مع تكليف موجود بنفس الاسم والمادة."""
    try:
        ...
        for i, row in enumerate(rows[1:], start=2):
            if safe_get(row, 0) == subject and safe_get(row, 1) == name:
                ...
                invalidate_data_cache("assignments")
                return True
...
def replace_task_content(subject, name, text_val, file_ids):
    """يستبدل نص وملفات تكليف موجود بالكامل. لو ما أُرسلت ملفات جديدة، يبقي الملفات القديمة كما هي."""
    try:
        rows = assignments_sheet.get_all_values()
        for i, row in enumerate(rows[1:], start=2):
            if safe_get(row, 0) == subject and safe_get(row, 1) == name:
...
def replace_summary_content(subject, old_label, new_label, file_ids):
    """يستبدل عنوان وملفات ملخص موجود بالكامل. لو ما أُرسلت ملفات جديدة، يبقي الملفات القديمة كما هي."""
    try:
        ...
        for i, row in enumerate(rows[1:], start=2):
            if safe_get(row, 0) == subject and safe_get(row, 2) == old_label:
                ...
                invalidate_data_cache("summaries")
                return True
        return False
    except Exception as e:
...
def _send_task_content(chat_id, task):
    msg = f"📋 *{task['name']}*"
    if task["text"]:
        msg += f"\n{task['text']}"
    if task["alert"]:
        msg += f"\n⚠️ {task['alert']}"
    if task["file_ids"]:
        send_files_with_text(chat_id, msg, task["file_ids"])
    else:
        bot.send_message(chat_id, msg, parse_mode="Markdown")

def show_subject_tasks(chat_id, uid, subject):
    tasks = get_tasks_for_subject(subject)
    if not tasks:
        bot.send_message(chat_id, f"لا توجد تكاليف لـ *{subject}*", parse_mode="Markdown",
                         reply_markup=subject_options_menu(uid))
        return
    if len(tasks) == 1:
        _send_task_content(chat_id, tasks[0])
        return
    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    for t in tasks:
        m.add(t["name"])
    m.add(_make_btn("زر_عوده", uid))
    user_state[uid] = {"browsing_tasks": True, "subject": subject}
    bot.send_message(chat_id, f"📋 تكاليف *{subject}*:", parse_mode="Markdown", reply_markup=m)

def _reset_timer(uid, key, fn):
...
def get_lecture_subjects():
    """يرجع كل المواد الموجودة بصفحة 'المحاضرات' الجديدة (بدون تكرار) — تُستخدم كقائمة 'كل المواد' عند إضافة ملخص."""
    try:
        seen, result = set(), []
...
def get_subjects_with_summaries():
    """يرجع أسماء المواد التي لديها ملخص واحد على الأقل، بترتيب أول ظهور بالشيت."""
    try:
        seen = []
...
def get_summaries_for_subject(subject):
    """يرجع ملخصات مادة معينة. النص المعروض كزر = عمود 'الملخصات' نفسه."""
    try:
        items = []
        for r in get_tab_data("summaries"):
            if safe_get(r, 0) == subject:
...
def get_known_students():
    """يرجع أسماء الطلاب (صانعي الملخصات) بدون تكرار."""
    try:
        seen = []
        for r in get_tab_data("summaries"):
            s = safe_get(r, 1)
...
def save_summary(subject, student, label, file_ids=None):
    """يضيف صف ملخص جديد لصفحة 'الملخصات'."""
    try:
        file_ids = file_ids or []
...
def _send_summary_content(chat_id, item):
    msg = f"📖 *{item['label']}*"
    if item["student"]:
        msg += f"\n✍️ {item['student']}"
    if item["file_ids"]:
        send_files_with_text(chat_id, msg, item["file_ids"])
    else:
        bot.send_message(chat_id, msg, parse_mode="Markdown")

def show_subject_summaries(chat_id, uid, subject):
    items = get_summaries_for_subject(subject)
    if not items:
        bot.send_message(chat_id, f"لا توجد ملخصات لـ *{subject}*", parse_mode="Markdown",
                         reply_markup=subject_options_menu(uid))
        return
    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    for it in items:
        m.add(it["label"])
    m.add(_make_btn("زر_عوده", uid))
    user_state[uid] = {"browsing_summaries": True, "subject": subject}
    bot.send_message(chat_id, f"📖 ملخصات *{subject}*:", parse_mode="Markdown", reply_markup=m)

def _show_student_picker(chat_id, uid):
    students = get_known_students()
    m = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    for s in students:
        m.add(s)
    m.add(bt("زر_اضافة_طالب", uid))
    m.add(_make_btn("زر_عوده", uid))
    user_state[uid]["step"] = "choose_summary_student"
    bot.send_message(chat_id, "👤 اختر اسم الطالب (صانع الملخص):", reply_markup=m)

def _finalize_summary(chat_id, uid, admin, owner, student_name):
    state = user_state.get(uid, {})
    subj = state.get("subject", "")
    label = state.get("summary_label", "").strip() or student_name
    fids = [f["file_id"] for f in state.get("pending_files", [])]
    ok = save_summary(subj, student_name, label, file_ids=fids)
    bot.send_message(chat_id, bt("رسالة_تم_الحفظ", uid) if ok else bt("رسالة_خطأ", uid),
                     reply_markup=main_menu(uid, admin=admin, owner=owner))
    user_state.pop(uid, None)

def get_subjects_with_booklets():
    """يرجع أسماء المواد التي لديها ملزمة واحدة على الأقل، بترتيب أول ظهور بالشيت."""
    try:
        seen = []
...
def get_booklets_for_subject(subject):
    """يرجع ملازم مادة معينة بترتيب الإدخال."""
    try:
        items = []
        for r in get_tab_data("booklets"):
            if safe_get(r, 0) == subject:
...
def replace_booklet_content(subject, old_name, new_name, file_ids, price=None):
    """يستبدل اسم وملف ملزمة موجودة. لو ما أُرسلت ملفات جديدة، يبقي الملف القديم. السعر يبقى كما هو إلا لو أُرسل صراحة."""
    try:
        rows = booklets_sheet.get_all_values()
        for i, row in enumerate(rows[1:], start=2):
...
def _send_booklet_content(chat_id, item):
    msg = f"📋 *{item['name']}*"
    if item["price"]:
        msg += f"\n💰 {item['price']}"
    if item["file_ids"]:
...
def show_subject_booklets(chat_id, uid, subject):
    items = get_booklets_for_subject(subject)
    if not items:
        bot.send_message(chat_id, f"لا توجد ملازم لـ *{subject}*", parse_mode="Markdown",
                         reply_markup=subject_options_menu(uid))
        return
    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    for it in items:
        m.add(it["name"])
    m.add(_make_btn("زر_عوده", uid))
    user_state[uid] = {"browsing_booklets": True, "subject": subject}
    bot.send_message(chat_id, f"📋 ملازم *{subject}*:", parse_mode="Markdown", reply_markup=m)

def get_subjects_with_exams():
    """يرجع أسماء المواد التي لديها نموذج اختبار واحد على الأقل، بترتيب أول ظهور بالشيت."""
    try:
        seen = []
...
def get_exams_for_subject(subject):
    """يرجع نماذج اختبار مادة معينة بترتيب الإدخال."""
    try:
        items = []
        for r in get_tab_data("exams"):
            if safe_get(r, 0) == subject:
...
def replace_exam_content(subject, old_name, new_name, file_ids):
    """يستبدل اسم و/أو ملف نموذج اختبار موجود. لو ما أُرسلت ملفات جديدة، يبقي الملف القديم."""
    try:
        rows = exams_sheet.get_all_values()
        for i, row in enumerate(rows[1:], start=2):
...
def _send_exam_content(chat_id, item):
    msg = f"🧾 *{item['name']}*"
    if item["file_ids"]:
        send_files_with_text(chat_id, msg, item["file_ids"])
    else:
...
def show_subject_exams(chat_id, uid, subject):
    items = get_exams_for_subject(subject)
    if not items:
        bot.send_message(chat_id, f"لا توجد نماذج اختبارات لـ *{subject}*", parse_mode="Markdown",
                         reply_markup=subject_options_menu(uid))
        return
    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    for it in items:
        m.add(it["name"])
    m.add(_make_btn("زر_عوده", uid))
    user_state[uid] = {"browsing_exams": True, "subject": subject}
    bot.send_message(chat_id, f"🧾 نماذج اختبارات *{subject}*:", parse_mode="Markdown", reply_markup=m)

def get_notes():
    """يقرأ الملازم من صفحة 'الملازم' الجديدة.
    عمود A = المادة، عمود B = سعر الملزمة، عمود C = الملازم (الاسم/الوصف).
    يرجع {subject: [(note, price), ...]} — السعر يُعرض جنب اسم الملزمة."""
    try:
        notes = {}  # {subject: [(note, price), ...]}
        for r in get_tab_data("booklets"):
            subj  = r[0].strip() if len(r) > 0 else ""
            ...
            if subj and note:
                if subj not in notes:
                    notes[subj] = []
                entry = (note, price)
                if entry not in notes[subj]:
                    notes[subj].append(entry)
        return notes
    except Exception as e:
...
def delete_lecture(date, subject):
    """يحذف صف محاضرة كامل من صفحة 'المحاضرات' الجديدة."""
    try:
        rows = lectures_sheet.get_all_values()
        for i, row in enumerate(rows[1:], start=2):
            if safe_get(row, 0) and parse_date(safe_get(row, 0)) == date and safe_get(row, 1) == subject:
...
def save_lecture_time(date, subject, new_time):
    """يحدّث وقت محاضرة موجودة فقط (عمود C بصفحة 'المحاضرات')."""
    try:
        rows = lectures_sheet.get_all_values()
        for i, row in enumerate(rows[1:], start=2):
            if safe_get(row, 0) and parse_date(safe_get(row, 0)) == date and safe_get(row, 1) == subject:
...
def get_data_summary_for_ai(uid, user_role):
    ...
    if not data and not get_tab_data("lectures") and not get_tab_data("assignments") and not get_tab_data("summaries") and not get_tab_data("booklets") and not get_tab_data("exams"):
    ...
    for row in data:
        ...
        if subject not in subjects:
            subjects[subject] = {"lectures": [], "tasks": [], "summaries": [], "booklets": [], "exams": []}
    ...
    for row in get_tab_data("assignments"):
        ...
        desc = f"{name}" + (f": {txt}" if txt else "") + (" 📎" if has_file else "")
        subjects[subject]["tasks"].append(desc)

    for row in get_tab_data("summaries"):
        subject = safe_get(row, 0)
        ...
        desc = f"{label}" + (f" — ✍️ {student}" if student else "")
        subjects[subject]["summaries"].append(desc)

    for row in get_tab_data("booklets"):
        subject = safe_get(row, 0)
        name = safe_get(row, 2)
        if not subject or not name:
            continue
        ...
        desc = f"{name}" + (f" — 💰 {price}" if price else "") + (" 📎" if has_file else "")
        subjects[subject]["booklets"].append(desc)

    for row in get_tab_data("exams"):
        subject = safe_get(row, 0)
        name = safe_get(row, 1)
        if not subject or not name:
            continue
...
def get_bot_code_summary(uid):
    ...
    B = {k: bt(v, uid) for k, v in {
        ...
        "اضافة_ملزمه":  "زر_اضافة_ملزمه",
        "تعديل_ملزمه":  "زر_تعديل_ملزمه",
        "نماذج":         "زر_نماذج_الاختبارات",
        "اضافة_نموذج":  "زر_اضافة_نموذج",
        "تعديل_نموذج":  "زر_تعديل_نموذج",
    ...
    lines.append(f"• {B['اضافة_نموذج']}: المادة (من صفحة المحاضرات) ← نوع النموذج (الاسم) ← أرسل الملف ← حفظ تلقائي")

    lines.append(f"\n## نماذج الاختبارات ##")
    lines.append(f"{B['نماذج']}: يعرض المواد التي لديها نماذج، ثم أسماء النماذج — الضغط يرسل الملف مباشرة")

    lines.append(f"\n## تعديل أو حذف ##")
    lines.append(f"{B['تعديل']} ← اختر ({B['تعديل_محاضره']} / {B['تعديل_تكليف']} / {B['تعديل_ملخص']} / {B['تعديل_ملزمه']} / {B['تعديل_نموذج']}) ← المادة ← تعديل أو حذف")
...
def _get_button_description(key):
    ...
    desc = {
        "زر_المواد":       "يعرض قائمة المواد. اختر مادة لعرض جدولها، تكاليفها، ملخصاتها، أو سعرها مع ملزمتها.",
        "زر_التكاليف":     "يعرض المواد التي لديها تكاليف. اختر مادة لعرض تكاليفها بالاسم.",
        "زر_الجدول":       "محاضرات آخر يوم دراسي.",
        "زر_الملخصات":     "آخر الملخصات المضافة.",
        "زر_الملازم":      "يعرض المواد التي لديها ملازم، ثم أسماء الملازم بالتفصيل.",
        "زر_نماذج_الاختبارات": "يعرض المواد التي لديها نماذج اختبارات، ثم أسماء النماذج.",
        ...
        "زر_اضافة_محاضره": "للأدمن: إضافة محاضرة مباشرة من القائمة الرئيسية.",
        "زر_اضافة":        "للأدمن: إضافة تكليف، ملخص، ملزمة، أو نموذج اختبار.",
...
def lecture_subjects_kb(uid):
    """قائمة كل المواد من صفحة 'المحاضرات' — تُستخدم عند إضافة ملخص (بدون استثناء)."""
...
def try_execute_admin_command(text, uid, user_role, chat_id, bot_instance):
    ...
    # إضافة تكليف (بالاسم، بدون تاريخ — الملفات تُرفع عبر زر الإضافة فقط)
    pattern_task = r'(?:أضف|إضافة)\s*(?:تكليف|واجب)\s*(?:مادة\s*)?([^\s]+)\s*(?:اسم\s*)?([^\n]+?)\s*(?:نص\s*)(.+)'
    m = re.search(pattern_task, text, re.IGNORECASE)
    if m:
        subject = m.group(1).strip()
        task_name = m.group(2).strip()
        task_text = m.group(3).strip()
        if save_task(subject, task_name, text_val=task_text):
            log_data_addition(uid, "تكليف", f"{subject} - {task_name}")
    ...
    # حذف تكليف (بالاسم، بدون تاريخ)
    pattern_delete_task = r'(?:احذف|حذف)\s*تكليف\s*(?:مادة\s*)?([^\s]+)\s*(?:اسم\s*)?(.+)'
    m = re.search(pattern_delete_task, text, re.IGNORECASE)
    if m:
        subject = m.group(1).strip()
        task_name = m.group(2).strip()
        if delete_task(subject, task_name):
            return True, f"✅ تم حذف التكليف «{task_name}» للمادة {subject}"
        else:
            return True, f"❌ لم يتم العثور على تكليف «{task_name}» للمادة {subject}"

    # حذف عنصر
    pattern_delete = r'(?:احذف|حذف)\s*(محاضرة|ملخص|تنبيه)\s*(?:مادة\s*)?([^\s]+)\s*(?:تاريخ\s*)?(\d{1,2}/\d{1,2}/\d{4})'
    ...
    if m:
        ...
        if type_word == "محاضرة":
            if delete_lecture(date, subject):
...
def _handle_internal_cmd(cmd: str) -> str:
    ...
    if cmd == "refresh_texts":
        ...
        invalidate_data_cache()  # يشمل الست صفحات الجديدة كلها
        for k in _DATA_TABS:
            get_tab_data(k)  # يبني كاش كل صفحة فوراً
        return "✅ بيانات الشيت محدّثة (القديمة + الست صفحات الجديدة)"
        ...
        tabs_status = " | ".join(
            f"{k}:{'✅' if _data_cache[k]['data'] else '⭕'}" for k in _DATA_TABS
        )
        return (
            f"📊 حالة البوت\n"
            f"🤖 AI providers: {len(AI_PROVIDERS)}\n"
            f"👥 cache المستخدمين: {'محدّث' if _users_cache['data'] else 'فارغ'} ({users_age}ث)\n"
            f"📋 cache البيانات القديمة: {'محدّث' if _sheet_data_cache['data'] else 'فارغ'} ({data_age}ث)\n"
            f"📁 cache الصفحات الجديدة: {tabs_status}\n"
...
def handle_file(message):
    ...
    if state.get("adding_data") and state.get("step") == "enter_task_content":
        cap = (message.caption or "").strip()
        user_state[uid].setdefault("pending_files", []).append({"file_id": file_id, "file_type": ftype})
        if cap:
            prev = user_state[uid].get("task_text", "")
            user_state[uid]["task_text"] = (prev + "\n" + cap).strip() if prev else cap

        def _finish_task_content():
            st = user_state.get(uid, {})
            if st.get("step") == "enter_task_content":
                user_state[uid]["step"] = "task_content_ready"
                mk = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
                mk.row(bt("زر_تنبيه_اضافي", uid), bt("زر_حفظ", uid))
                mk.add(back_btn)
                bot.send_message(message.chat.id, "📋 جاهز — اضغط *حفظ*، أو أضف تنبيه.", parse_mode="Markdown", reply_markup=mk)
    ...
    if state.get("adding_data") and state.get("step") == "enter_booklet_file":
        ...
        mk = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
        mk.row(bt("زر_حفظ", uid), bt("زر_اضافة_سعر_ملزمة", uid))
        mk.add(back_btn)
        bot.send_message(message.chat.id, "📎 تم استلام الملف.", reply_markup=mk)
        return

    if state.get("adding_data") and state.get("step") == "enter_exam_file":
        subj = state.get("subject", "")
        name = state.get("exam_name", "")
        ok = save_exam(subj, name, file_ids=[file_id])
        bot.send_message(message.chat.id, bt("رسالة_تم_الحفظ", uid) if ok else bt("رسالة_خطأ", uid),
                         reply_markup=main_menu(uid, admin=f_admin, owner=f_owner))
        user_state.pop(uid, None)
        return

    if state.get("adding_data") and state.get("step") == "enter_summary_content":
        cap = (message.caption or "").strip()
        user_state[uid].setdefault("pending_files", []).append({"file_id": file_id, "file_type": ftype})
        if cap:
            prev = user_state[uid].get("summary_label", "")
            user_state[uid]["summary_label"] = (prev + "\n" + cap).strip() if prev else cap

        def _finish_summary_content():
            st = user_state.get(uid, {})
            if st.get("step") == "enter_summary_content":
                _show_student_picker(message.chat.id, uid)
    ...
    if state.get("editing_data") and state.get("step") == "enter_replace_content":
        ...
        user_state[uid].setdefault("pending_files", []).append({"file_id": file_id, "file_type": ftype})
        if cap:
            prev = user_state[uid].get("replace_text", "")
            user_state[uid]["replace_text"] = (prev + "\n" + cap).strip() if prev else cap

        def _finish_replace_content():
            st = user_state.get(uid, {})
            if st.get("step") == "enter_replace_content":
                user_state[uid]["step"] = "confirm_replace"
                mk = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
                mk.row(bt("زر_استبدال", uid), bt("زر_الغاء", uid))
    ...
    if state.get("requesting_upload") and state.get("step") in ("waiting_files_req", "confirm_req"):
        ...
        _reset_timer(uid, "requesting_upload", _finish_req)
        return
    ...
    if state.get("uploading_help") and state.get("step") == "waiting_file_help":
        ...
        _reset_timer(uid, "uploading_help", _finish_help)
        return
...
def handle_message(message):
    ...
    try:
        ...
        SUBJ_OPTS = [bt(k, uid) for k in ["خيار_الجدول", "خيار_التكاليف", "خيار_الملخص", "خيار_الملزمه", "خيار_نماذج_الاختبارات"]]
        if _free and state.get("subject") and text in SUBJ_OPTS:
            ...
            if text == bt("خيار_الملزمه", uid):
                show_subject_booklets(message.chat.id, uid, subj)
                return
            if text == bt("خيار_نماذج_الاختبارات", uid):
                show_subject_exams(message.chat.id, uid, subj)
                return
            if text == bt("خيار_الجدول", uid):
                lect_rows_s = [r for r in get_tab_data("lectures") if safe_get(r, 1) == subj]
                dates = list(dict.fromkeys(
                    parse_date(safe_get(r, 0)) for r in lect_rows_s
                    if get_text(safe_get(r, 2)) and safe_get(r, 0)))
                if not dates:
                    bot.send_message(message.chat.id, f"لا توجد محاضرات لـ *{subj}*",
                                     parse_mode="Markdown", reply_markup=subject_options_menu(uid))
                    return
                user_state[uid] = {"subject": subj, "action": text, "awaiting_date": True,
                                    "col": 2, "dates": dates, "tab": "lectures"}
                bot.send_message(message.chat.id, "📅 اختر التاريخ:", reply_markup=dates_menu_kb(dates, uid))
                return
            if text == bt("خيار_التكاليف", uid):
                show_subject_tasks(message.chat.id, uid, subj)
                return
            if text == bt("خيار_الملخص", uid):
                show_subject_summaries(message.chat.id, uid, subj)
                return
            return
        ...
        if state.get("awaiting_date"):
            ...
            src = get_tab_data("lectures") if state.get("tab") == "lectures" else data
            matched = [r for r in src if safe_get(r, 1) == subj and parse_date(safe_get(r, 0)) == text]
            if not matched:
        ...
        if text == bt("زر_التكاليف", uid):
            subs = get_subjects_with_tasks()
            if not subs:
                bot.send_message(message.chat.id, "📭 لا توجد تكاليف حالياً.", reply_markup=main_menu(uid, admin=admin, owner=owner))
                return
            if len(subs) == 1:
                show_subject_tasks(message.chat.id, uid, subs[0])
                return
            m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
            for s in subs:
                m.add(s)
            m.add(_make_btn("زر_عوده", uid))
            user_state[uid] = {"choosing_task_subject": True}
            bot.send_message(message.chat.id, "📌 اختر المادة:", reply_markup=m)
            return

        if state.get("choosing_task_subject") and text in get_subjects_with_tasks():
            user_state.pop(uid, None)
            show_subject_tasks(message.chat.id, uid, text)
            return

        if state.get("browsing_tasks"):
            subj = state.get("subject", "")
            tasks = get_tasks_for_subject(subj)
            match = next((t for t in tasks if t["name"] == text), None)
            if match:
                _send_task_content(message.chat.id, match)
                return
            # أي نص ثاني (غير اسم تكليف) يُعامل بشكل طبيعي — يخرج من القائمة تلقائياً بالمرور للأسفل

        if text == bt("زر_الجدول", uid):
            lect_data = get_tab_data("lectures")
            ld = get_last_date(lect_data, 2)
            if not ld:
                bot.send_message(message.chat.id, "📭 لا توجد محاضرات.", reply_markup=main_menu(uid, admin=admin, owner=owner))
                return
            rows_s = [r for r in lect_data if parse_date(safe_get(r, 0)) == ld and get_text(safe_get(r, 2))]
        ...
        if text == bt("زر_الملخصات", uid):
            subs = get_subjects_with_summaries()
            if not subs:
                bot.send_message(message.chat.id, "📭 لا توجد ملخصات حالياً.", reply_markup=main_menu(uid, admin=admin, owner=owner))
                return
            if len(subs) == 1:
                show_subject_summaries(message.chat.id, uid, subs[0])
                return
            m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
            for s in subs:
                m.add(s)
            m.add(_make_btn("زر_عوده", uid))
            user_state[uid] = {"choosing_summary_subject": True}
            bot.send_message(message.chat.id, "📌 اختر المادة:", reply_markup=m)
            return

        if state.get("choosing_summary_subject") and text in get_subjects_with_summaries():
            user_state.pop(uid, None)
            show_subject_summaries(message.chat.id, uid, text)
            return

        if state.get("browsing_summaries"):
            subj = state.get("subject", "")
            items = get_summaries_for_subject(subj)
            match = next((it for it in items if it["label"] == text), None)
            if match:
                _send_summary_content(message.chat.id, match)
                return

        if text == bt("زر_الملازم", uid):
            subs = get_subjects_with_booklets()
            if not subs:
                bot.send_message(message.chat.id, "📭 لا توجد ملازم حالياً.", reply_markup=main_menu(uid, admin=admin, owner=owner))
                return
            if len(subs) == 1:
                show_subject_booklets(message.chat.id, uid, subs[0])
                return
            m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
            for s in subs:
                m.add(s)
            m.add(_make_btn("زر_عوده", uid))
            user_state[uid] = {"choosing_booklet_subject": True}
            bot.send_message(message.chat.id, "📌 اختر المادة:", reply_markup=m)
            return

        if state.get("choosing_booklet_subject") and text in get_subjects_with_booklets():
            user_state.pop(uid, None)
            show_subject_booklets(message.chat.id, uid, text)
            return

        if state.get("browsing_booklets"):
            subj = state.get("subject", "")
            items = get_booklets_for_subject(subj)
            match = next((it for it in items if it["name"] == text), None)
            if match:
                _send_booklet_content(message.chat.id, match)
                return

        if text == bt("زر_نماذج_الاختبارات", uid):
            subs = get_subjects_with_exams()
            if not subs:
                bot.send_message(message.chat.id, "📭 لا توجد نماذج اختبارات حالياً.", reply_markup=main_menu(uid, admin=admin, owner=owner))
                return
            if len(subs) == 1:
                show_subject_exams(message.chat.id, uid, subs[0])
                return
            m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
            for s in subs:
                m.add(s)
            m.add(_make_btn("زر_عوده", uid))
            user_state[uid] = {"choosing_exam_subject": True}
            bot.send_message(message.chat.id, "📌 اختر المادة:", reply_markup=m)
            return

        if state.get("choosing_exam_subject") and text in get_subjects_with_exams():
            user_state.pop(uid, None)
            show_subject_exams(message.chat.id, uid, text)
            return

        if state.get("browsing_exams"):
            subj = state.get("subject", "")
            items = get_exams_for_subject(subj)
            match = next((it for it in items if it["name"] == text), None)
            if match:
                _send_exam_content(message.chat.id, match)
                return
        ...
        if text == bt("زر_اضافة_محاضره", uid):
            if not (admin or owner):
                bot.send_message(message.chat.id, bt("رسالة_ادمن_فقط", uid))
                return
            user_state[uid] = {"adding_data": True, "data_type": "lecture", "step": "enter_date"}
            bot.send_message(message.chat.id, "📅 أدخل تاريخ المحاضرة:", reply_markup=date_suggestions_menu(for_lecture=True, uid=uid))
            return

        if state.get("adding_data"):
            step = state.get("step", "")
            ADD_MAP = {bt("زر_اضافة_تكليف", uid): "task",
                       bt("زر_اضافة_ملخص", uid): "summary",
                       bt("زر_اضافة_ملزمه", uid): "notes",
                       bt("زر_اضافة_نموذج", uid): "exam"}
            if step == "choose_type" and text in ADD_MAP:
                dtype = ADD_MAP[text]
                user_state[uid]["data_type"] = dtype
                user_state[uid]["step"] = "choose_subject"
                if dtype in ("summary", "exam"):
                    _kb_no, _ = lecture_subjects_kb(uid)
                else:
            ...
            if step == "choose_subject" and (text in subjects_list or (state.get("data_type") in ("summary", "exam") and text in get_lecture_subjects())):
                ...
                if dtype == "lecture":
                    ...
                    user_state[uid]["step"] = "enter_booklet_name"
                    bot.send_message(message.chat.id, "✏️ أدخل اسم الملزمة:", reply_markup=back_only_menu(uid))
                elif dtype == "exam":
                    user_state[uid]["step"] = "enter_exam_name"
                    bot.send_message(message.chat.id, "✏️ أدخل نوع النموذج (الاسم):", reply_markup=back_only_menu(uid))
                elif dtype == "task":
                    user_state[uid]["step"] = "enter_task_name"
                    bot.send_message(message.chat.id, "✏️ أدخل اسم التكليف:", reply_markup=back_only_menu(uid))
                elif dtype == "summary":
                    user_state[uid]["step"] = "enter_summary_content"
                    user_state[uid]["pending_files"] = []
                    user_state[uid]["summary_label"] = ""
                    bot.send_message(message.chat.id, "📎 أرسل ملف الملخص (يمكن إرسال أكثر من ملف):", reply_markup=back_only_menu(uid))
                else:
                    user_state[uid]["step"] = "enter_date"
            ...
            if step == "enter_summary_content":
                prev = state.get("summary_label", "")
            ...
            if step == "choose_summary_student":
                if text == bt("زر_اضافة_طالب", uid):
                    user_state[uid]["step"] = "enter_new_student_name"
                    bot.send_message(message.chat.id, "✏️ اكتب اسم الطالب الجديد:", reply_markup=back_only_menu(uid))
                    return
                _finalize_summary(message.chat.id, uid, admin, owner, text)
                return

            if step == "enter_new_student_name":
                if not text.strip():
                    bot.send_message(message.chat.id, "✏️ اكتب اسم الطالب الجديد:", reply_markup=back_only_menu(uid))
                    return
                _finalize_summary(message.chat.id, uid, admin, owner, text.strip())
                return

            if step == "enter_exam_name":
                if not text.strip():
                    bot.send_message(message.chat.id, "✏️ أدخل نوع النموذج (الاسم):", reply_markup=back_only_menu(uid))
                    return
                user_state[uid]["exam_name"] = text.strip()
                user_state[uid]["step"] = "enter_exam_file"
                bot.send_message(message.chat.id, "📎 أرسل ملف النموذج:", reply_markup=back_only_menu(uid))
                return

            if step == "enter_booklet_name":
                if not text.strip():
                    bot.send_message(message.chat.id, "✏️ أدخل اسم الملزمة:", reply_markup=back_only_menu(uid))
                    return
                user_state[uid]["booklet_name"] = text.strip()
                user_state[uid]["step"] = "enter_booklet_file"
                bot.send_message(message.chat.id, "📎 أرسل ملف الملزمة (PDF أو أي ملف مدعوم):", reply_markup=back_only_menu(uid))
                return

            if step == "booklet_content_ready":
                if text == bt("زر_حفظ", uid):
                    subj = state.get("subject", "")
                    name = state.get("booklet_name", "")
                    fids = [f["file_id"] for f in state.get("pending_files", [])]
                    ok = save_booklet(subj, name, file_ids=fids)
                    bot.send_message(message.chat.id, bt("رسالة_تم_الحفظ", uid) if ok else bt("رسالة_خطأ", uid),
                                     reply_markup=main_menu(uid, admin=admin, owner=owner))
                    user_state.pop(uid, None)
                    return
                elif text == bt("زر_اضافة_سعر_ملزمة", uid):
                    user_state[uid]["step"] = "enter_booklet_price"
                    bot.send_message(message.chat.id, "💰 أدخل سعر الملزمة:", reply_markup=back_only_menu(uid))
                    return
                return

            if step == "enter_booklet_price":
                subj = state.get("subject", "")
                name = state.get("booklet_name", "")
                fids = [f["file_id"] for f in state.get("pending_files", [])]
                ok = save_booklet(subj, name, file_ids=fids, price=text.strip())
                bot.send_message(message.chat.id, bt("رسالة_تم_الحفظ", uid) if ok else bt("رسالة_خطأ", uid),
                                 reply_markup=main_menu(uid, admin=admin, owner=owner))
                user_state.pop(uid, None)
                return
            ...
            if step == "enter_task_name":
                if not text.strip():
                    bot.send_message(message.chat.id, "✏️ أدخل اسم التكليف:", reply_markup=back_only_menu(uid))
                    return
                user_state[uid]["task_name"] = text.strip()
                user_state[uid]["step"] = "enter_task_content"
                user_state[uid]["pending_files"] = []
                user_state[uid]["task_text"] = ""
                bot.send_message(message.chat.id,
                                 "📎 أرسل نص التكليف و/أو الملفات دفعة واحدة (يمكن إرسال أكثر من ملف):",
                                 reply_markup=back_only_menu(uid))
                return

            if step == "enter_task_content":
                prev = state.get("task_text", "")
                user_state[uid]["task_text"] = (prev + "\n" + text).strip() if prev else text

                def _finish_task_content_txt():
                    st = user_state.get(uid, {})
                    if st.get("step") == "enter_task_content":
                        user_state[uid]["step"] = "task_content_ready"
                        mk = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
                ...
                _reset_timer(uid, "task_content", _finish_task_content_txt)
                return

            if step == "task_content_ready":
                if text == bt("زر_حفظ", uid):
                    subj = state.get("subject", "")
                    name = state.get("task_name", "")
                    task_text = state.get("task_text", "")
                    fids = [f["file_id"] for f in state.get("pending_files", [])]
                    ok = save_task(subj, name, text_val=task_text, file_ids=fids)
                    bot.send_message(message.chat.id, bt("رسالة_تم_الحفظ", uid) if ok else bt("رسالة_خطأ", uid),
                                     reply_markup=main_menu(uid, admin=admin, owner=owner))
                    user_state.pop(uid, None)
                    return
                elif text == bt("زر_تنبيه_اضافي", uid):
                    user_state[uid]["step"] = "enter_task_alert"
                    bot.send_message(message.chat.id, "⚠️ اكتب نص التنبيه:", reply_markup=back_only_menu(uid))
                    return
                return

            if step == "enter_task_alert":
                user_state[uid]["task_alert"] = text
                user_state[uid]["step"] = "confirm_task_alert"
                mk = telebot.types.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
                mk.row(bt("زر_حفظ", uid), bt("زر_الغاء", uid))
                bot.send_message(message.chat.id, f"⚠️ التنبيه: *{text}*", parse_mode="Markdown", reply_markup=mk)
                return

            if step == "confirm_task_alert":
                subj = state.get("subject", "")
                name = state.get("task_name", "")
                task_text = state.get("task_text", "")
                fids = [f["file_id"] for f in state.get("pending_files", [])]
                alert_val = state.get("task_alert", "") if text == bt("زر_حفظ", uid) else ""
                ok = save_task(subj, name, text_val=task_text, file_ids=fids, alert=alert_val)
                bot.send_message(message.chat.id, bt("رسالة_تم_الحفظ", uid) if ok else bt("رسالة_خطأ", uid),
                                 reply_markup=main_menu(uid, admin=admin, owner=owner))
                user_state.pop(uid, None)
                return
        ...
        if state.get("editing_data"):
            ...
            EDIT_MAP = {bt("زر_تعديل_محاضره", uid): "lecture", bt("زر_تعديل_تكليف", uid): "task",
                        bt("زر_تعديل_ملخص", uid): "summary", bt("زر_تعديل_ملزمه", uid): "notes",
                        bt("زر_تعديل_نموذج", uid): "exam"}
            COL_MAP = {"lecture": 2, "task": 4, "summary": 6, "notes": 8}
            ...
            if step == "choose_subject" and text in subjects_list:
                ...
                if dtype == "price":
                    ...
                    user_state[uid]["date"] = ""
                    bot.send_message(message.chat.id, f"*سعر الملزمة* الحالي: *{current or 'فارغ'}*", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                elif dtype == "notes":
                    books = get_booklets_for_subject(text)
                    if not books:
                    ...
                    if len(books) == 1:
                        bk = books[0]
                        user_state[uid]["step"] = "choose_action"
                        user_state[uid]["booklet_name_key"] = bk["name"]
                        user_state[uid]["current_val"] = bk["name"]
                        user_state[uid]["date"] = ""
                        bot.send_message(message.chat.id, f"*{bk['name']}*" + (f" — 💰 {bk['price']}" if bk['price'] else ""), parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                        return
                    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
                    for bk in books:
                        m.add(bk["name"])
                    m.add(_make_btn("زر_عوده", uid))
                    user_state[uid]["step"] = "choose_booklet_name"
                    bot.send_message(message.chat.id, "📋 اختر الملزمة:", reply_markup=m)
                    return
                elif dtype == "exam":
                    exams = get_exams_for_subject(text)
                    if not exams:
                        bot.send_message(message.chat.id, bt("رسالة_لا_بيانات", uid), reply_markup=edit_data_menu(uid))
                        user_state[uid] = {"editing_data": True, "step": "choose_type"}
                        return
                    if len(exams) == 1:
                        ex = exams[0]
                        user_state[uid]["step"] = "choose_action"
                        user_state[uid]["exam_name_key"] = ex["name"]
                        user_state[uid]["current_val"] = ex["name"]
                        user_state[uid]["date"] = ""
                        bot.send_message(message.chat.id, f"*{ex['name']}*", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                        return
                    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
                    for ex in exams:
                        m.add(ex["name"])
                    m.add(_make_btn("زر_عوده", uid))
                    user_state[uid]["step"] = "choose_exam_name"
                    bot.send_message(message.chat.id, "🧾 اختر النموذج:", reply_markup=m)
                    return
                elif dtype == "task":
                    tasks = get_tasks_for_subject(text)
                    if not tasks:
                        bot.send_message(message.chat.id, bt("رسالة_لا_بيانات", uid), reply_markup=edit_data_menu(uid))
                        user_state[uid] = {"editing_data": True, "step": "choose_type"}
                        return
                    if len(tasks) == 1:
                        t = tasks[0]
                        user_state[uid]["step"] = "choose_action"
                        user_state[uid]["task_name"] = t["name"]
                        user_state[uid]["current_val"] = t["text"]
                        user_state[uid]["date"] = ""
                        bot.send_message(message.chat.id, f"*{t['name']}* — النص الحالي: *{t['text'] or 'فارغ'}*", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                        return
                    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
                    for t in tasks:
                        m.add(t["name"])
                    m.add(_make_btn("زر_عوده", uid))
                    user_state[uid]["step"] = "choose_task_name"
                    bot.send_message(message.chat.id, "📋 اختر التكليف:", reply_markup=m)
                    return
                elif dtype == "summary":
                    items = get_summaries_for_subject(text)
                    if not items:
                        bot.send_message(message.chat.id, bt("رسالة_لا_بيانات", uid), reply_markup=edit_data_menu(uid))
                        user_state[uid] = {"editing_data": True, "step": "choose_type"}
                        return
                    if len(items) == 1:
                        it = items[0]
                        user_state[uid]["step"] = "choose_action"
                        user_state[uid]["summary_label_key"] = it["label"]
                        user_state[uid]["current_val"] = it["label"]
                        user_state[uid]["date"] = ""
                        bot.send_message(message.chat.id, f"*{it['label']}* — ✍️ {it['student']}", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                        return
                    m = telebot.types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
                    for it in items:
                        m.add(it["label"])
                    m.add(_make_btn("زر_عوده", uid))
                    user_state[uid]["step"] = "choose_summary_name"
                    bot.send_message(message.chat.id, "📖 اختر الملخص:", reply_markup=m)
                    return
                else:
                    src = get_tab_data("lectures") if dtype == "lecture" else data
                    matched = [r for r in src if safe_get(r, 1) == text]
            ...
            if step == "choose_task_name":
                ...
                if not match:
                    return
                user_state[uid]["step"] = "choose_action"
                user_state[uid]["task_name"] = match["name"]
                user_state[uid]["current_val"] = match["text"]
                user_state[uid]["date"] = ""
                bot.send_message(message.chat.id, f"*{match['name']}* — النص الحالي: *{match['text'] or 'فارغ'}*", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                return
            if step == "choose_summary_name":
                subj = state.get("subject", "")
                items = get_summaries_for_subject(subj)
                match = next((it for it in items if it["label"] == text), None)
                if not match:
                    return
                user_state[uid]["step"] = "choose_action"
                user_state[uid]["summary_label_key"] = match["label"]
                user_state[uid]["current_val"] = match["label"]
                user_state[uid]["date"] = ""
                bot.send_message(message.chat.id, f"*{match['label']}* — ✍️ {match['student']}", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                return
            if step == "choose_booklet_name":
                subj = state.get("subject", "")
                books = get_booklets_for_subject(subj)
                match = next((bk for bk in books if bk["name"] == text), None)
                if not match:
                    return
                user_state[uid]["step"] = "choose_action"
                user_state[uid]["booklet_name_key"] = match["name"]
                user_state[uid]["current_val"] = match["name"]
                user_state[uid]["date"] = ""
                bot.send_message(message.chat.id, f"*{match['name']}*" + (f" — 💰 {match['price']}" if match['price'] else ""), parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                return
            if step == "choose_exam_name":
                subj = state.get("subject", "")
                exams = get_exams_for_subject(subj)
                match = next((ex for ex in exams if ex["name"] == text), None)
                if not match:
                    return
                user_state[uid]["step"] = "choose_action"
                user_state[uid]["exam_name_key"] = match["name"]
                user_state[uid]["current_val"] = match["name"]
                user_state[uid]["date"] = ""
                bot.send_message(message.chat.id, f"*{match['name']}*", parse_mode="Markdown", reply_markup=edit_action_menu(uid))
                return
            ...
            if step == "choose_action":
                if text == bt("زر_تعديل_زرار", uid):
                    dtype = state.get("data_type", "")
                    if dtype in ("task", "summary", "notes", "exam"):
                        user_state[uid]["step"] = "enter_replace_content"
                        user_state[uid]["pending_files"] = []
                        user_state[uid]["replace_text"] = ""
                        bot.send_message(message.chat.id,
                                         "📎 أرسل النص و/أو الملف الجديد (سيستبدل القديم بالكامل):",
                                         reply_markup=back_only_menu(uid))
                    else:
            ...
            if step == "confirm_replace":
                dtype = state.get("data_type", "")
                subj = state.get("subject", "")
                new_text = state.get("replace_text", "")
                fids = [f["file_id"] for f in state.get("pending_files", [])]
                if text == bt("زر_استبدال", uid):
                    if dtype == "task":
                        ok = replace_task_content(subj, state.get("task_name", ""), new_text, fids)
                    elif dtype == "summary":
                        old_label = state.get("summary_label_key", "")
                        new_label = new_text.strip() or old_label
                        ok = replace_summary_content(subj, old_label, new_label, fids)
                    elif dtype == "notes":
                        old_name = state.get("booklet_name_key", "")
                        new_name = new_text.strip() or old_name
                        ok = replace_booklet_content(subj, old_name, new_name, fids)
                    elif dtype == "exam":
                        old_name = state.get("exam_name_key", "")
                        new_name = new_text.strip() or old_name
                        ok = replace_exam_content(subj, old_name, new_name, fids)
                    else:
                        ok = False
                    bot.send_message(message.chat.id, bt("رسالة_تم_التعديل", uid) if ok else bt("رسالة_خطأ", uid),
                                     reply_markup=main_menu(uid, admin=admin, owner=owner))
                else:
                    bot.send_message(message.chat.id, "تم الإلغاء.", reply_markup=main_menu(uid, admin=admin, owner=owner))
                user_state.pop(uid, None)
                return
            if step == "confirm_delete":
                if text == "✅ نعم، احذف":
                    ...
                    if dtype == "price":
                        ...
                        ok = delete_booklet(subj, state.get("booklet_name_key", ""))
                        ...
                        ok = delete_exam(subj, state.get("exam_name_key", ""))
                        bot.send_message(message.chat.id, bt("رسالة_تم_الحذف", uid) if ok else bt("رسالة_خطأ", uid), reply_markup=main_menu(uid, admin=admin, owner=owner))
                    elif dtype == "lecture":
                        ok = delete_lecture(date, subj)
                        bot.send_message(message.chat.id, bt("رسالة_تم_الحذف", uid) if ok else bt("رسالة_خطأ", uid), reply_markup=main_menu(uid, admin=admin, owner=owner))
                    elif dtype == "task":
                        ok = delete_task(subj, state.get("task_name", ""))
                        bot.send_message(message.chat.id, bt("رسالة_تم_الحذف", uid) if ok else bt("رسالة_خطأ", uid), reply_markup=main_menu(uid, admin=admin, owner=owner))
                    elif dtype == "summary":
                        ok = delete_summary(subj, state.get("summary_label_key", ""))
                        bot.send_message(message.chat.id, bt("رسالة_تم_الحذف", uid) if ok else bt("رسالة_خطأ", uid), reply_markup=main_menu(uid, admin=admin, owner=owner))
                    else:
                        ok = delete_cell(date, subj, col)
                        bot.send_message(message.chat.id, bt("رسالة_تم_الحذف", uid) if ok else bt("رسالة_خطأ", uid), reply_markup=main_menu(uid, admin=admin, owner=owner))
                    user_state.pop(uid, None)
                elif text == bt("زر_الغاء", uid):
                    user_state[uid]["step"] = "choose_action"
            ...
            if step == "enter_new_val":
                ...
                if dtype == "price":
                    ...
                    ok = save_lecture_time(date, subj, text)
...
def handle_sched_accept(call):
    ...
    for e in entries:
        ...
        try:
            all_rows = lectures_sheet.get_all_values()
